...
class ModeloCursos{
    ...
    static public function obtenerCursosCreados($idCliente) {
        $stmt = Conexion::conectar()->prepare("SELECT * FROM cursos WHERE id_creador = :id");
        $stmt->bindParam(":id", $idCliente, PDO::PARAM_INT);
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }

    static public function obtenerCursosComprados($idCliente) {
        $stmt = Conexion::conectar()->prepare("
            SELECT c.* FROM carrito_cursos cc
            INNER JOIN cursos c ON c.id = cc.id_curso
